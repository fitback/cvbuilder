...
| v0.7.1 | 2026-08-21 | 充值改当面付扫码支付（页内二维码）、移除手动付款码上传、部署超时加固 |
...
| 08/21 | 当面付下单返回 ACQ.ACCESS_FORBIDDEN | 商户账号未开通「当面付」产品权限（代码与签名正确，支付宝业务层拒绝）；已加友好报错，待账号侧开通 |
...
| P0 | 数据库生产密码修改 | 🔴 |
| P0 | 开通支付宝「当面付」产品权限 | 🔴 当前下单返回 ACQ.ACCESS_FORBIDDEN；支付宝商家中心开通后做 10 元真实支付实测 |
...
**刚完成**（2026-08-21）：v0.7.1 充值改当面付扫码支付
- 后端 precreate 下单返回 `qr_code`（删手写 RSA 签名），充值页页内渲染二维码 + 3s 轮询到账；移除手动付款码上传（后端接口/管理页区块/health 检查/卷）
- 部署超时上限 30m/45m → 60m/75m
- ⏳ **待办：支付宝商家中心开通「当面付」产品**（当前下单返回 ACQ.ACCESS_FORBIDDEN）；开通后做 10 元真实支付实测
